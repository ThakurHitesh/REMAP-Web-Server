...
for line in fh1:
   if line[0] == ">":
      headerListseq.append(line[1:].strip())
      if currentSeqseq != '':
         seqListseq.append(currentSeqseq)

      currentSeqseq = ''
   else:
      currentSeqseq += line.strip()
...
for line in fh2:
   if line[0] == ">":
      headerListref.append(line[1:].strip())
      if currentSeqref != '':
         seqListref.append(currentSeqref)

      currentSeqref = ''
   else:
      currentSeqref += line.strip()

seqListref.append(currentSeqref)
outCAI = open("basicutility/Scriptout/"+sys.argv[3]+"CAIcalculator.txt",'w')
#outCAI = open("CAIcalculator.txt",'w')
counting=0
CAIseqLIST= []
for xseq in seqListseq: 
	CAI_result=CAI(xseq, reference=seqListref)
	CAIseqLIST.append(headerListseq[counting]+"^^"+xseq+"^^"+str(round(CAI_result,4))+"\n")
	counting = counting + 1;
resultCAIvalue="".join(CAIseqLIST)
outCAI.write(resultCAIvalue)
