...
$outsidevar=sprintf "%-17s","Repeats";
...
for ($i = 0; $i < scalar (@count_motifs); $i++)
  ...
  $changevar=sprintf "%-17s",$count_motifs[$i];
  print OUT $changevar;
...
for ($i = 0; $i < scalar (@red_rev); $i++)
  ...
  $againchangevar=sprintf "%-17s",$red_rev[$i];
  print OUT $againchangevar;
  for ($j = $min_repeats; $j <= $max_repeats; $j++)
    {
    if ($j < $typrep{$typ}) {print OUT "\t-";next};
